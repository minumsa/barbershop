...
export async function PUT(request: Request, { params: { id } }: { params: { id: string } }) {
  try {
    ...
    if (!file || typeof file === 'string') {
      return NextResponse.json({ msg: 'no file' }, { status: 400 })
    }
    if (file.size > maxFileSizeKB * 1024 * 8) {
      return NextResponse.json({ msg: `no more than ${maxFileSizeKB} KB` }, { status: 400 })
    }
    ...
    const result = await BarberShopModel.updateOne({ _id: id }, { $set: { imgUrl: s3ObjectUrl } }).exec();
